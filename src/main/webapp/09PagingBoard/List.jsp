...
//전체페이지 수를 계산한다.(전체게시물의 갯수 /페이지당게시물갯수 = 결과의 올림처리)
int totalPage = (int)Math.ceil((double)totalCount/pageSize);

/*
목록에 첫 진입시에는 페이지 관련 파라미터가 없는 상태이므로 무조건 1page로 지정한다.
만약 파라미터 pageNum이 있다면 request 내장객체를 통해 받은 후 페이지번호로 지정한다.
List.jsp => 이와 같이 파라미터가 없는 상태일 때는 null
List.jsp?pageNum= => 이와 같이 파라미터가 있는데 값이 없을 때는 빈 값으로 체크된다.
따라서 아래 if문은 2개의 조건으로 구성해야 한다.
*/
int pageNum = 1; // 1. 이걸 버튼으로 얻어옴 
    ...
    <table border="1" width="90%">
        <tr align="center">
        	...
        	request.getRequestURI() : request내장객체를 통해 현재 페이지에서
        		HOST부분을 제외한 전체 경로명을 얻어올 수 있다. 여기서 얻은
