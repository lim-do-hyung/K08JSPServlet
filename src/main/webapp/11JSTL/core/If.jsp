...
<body>
	...
	<!-- 
	JSTL의 if 태그는 else 구문이 따로 없으므로 두개의 if 태그를 통해
	else와 같이 사용해야 한다.
	...
	<c:if test="${not empty param.user }">
		...
		<br />
		<!-- 
		아이디, 패스워드를 입력했을 때만 일치 여부를 판단한 후 result에 결과를
		저장한다. 두번째 if문에서 not을 이용해 else 구문을 만들면 된다.
		 -->
		<c:if test="${param.user eq 'kosmo' && param.pass eq '1234'}" var="result00">
			${param.user} 님 하이룽~ <br />
		</c:if>
		<c:if test="${ not result00 }">
			아이디와 비번을 확인하세요<br />
		</c:if>
