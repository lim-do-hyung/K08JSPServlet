...
<!-- 
	...
	속성정리
		...
		value : 속성값
		scope : 4가지 영역명을 지정. 미 지정시 가장 좁은 page영역을 지정.
 ...
 -->
	...
	겹쳐지는 경우가 발생하게 된다. 이 때는 value를 싱글쿼테이션으로 감싸서 겹쳐지지 않게
	처리해야 한다. -->
	<c:set var="personVar1" value='<%=new Person("박문수",50) %>'
	 scope="request"></c:set>
	 <!-- 자바 빈즈의 getter()를 통해 멤버변수의 값을 출력한다. -->
	 <ul>
	 	<li>이름 : ${requestScope.personVar1.name }</li>
	 	<li>나이 : ${personVar1.age }</li>
	 </ul>
	 
	 <!-- 빈 객체를 생성한 후 target, property를 통해 멤버변수의 값을 지정한다. -->
